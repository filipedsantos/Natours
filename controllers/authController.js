...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // GRANT access to protected route (next() lead to next middleware -> getAllTours())
...
exports.restrictTo = (...roles) =>
  catchAsync(async (req, res, next) => {
    ...
    // req.user.role comes from the last middleware call (protect -> req.user = currentUser;)
    // because that middleware happens right before this and put the user on the req object!
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('There is no user with the inserted email!', 404));
  }
